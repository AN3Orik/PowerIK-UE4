...
void FPowerIKCore::LoadBonesFromControlRig(FRigBoneHierarchy* Hierarchy)
{
	// reset all data structures
	Reset();
	...
	for (int i = 0; i < Hierarchy->Num(); ++i)
	{
		...
		FPowerIKBoneData& Bone = BonesData[i];
		Bone.BoneName = RigBone.Name;
		CopyFNameToString(Bone.BoneName, Bone.NameString);
		Bone.Index = RigBone.Index;
		Bone.ParentIndex = RigBone.ParentIndex;

		// store in the map for later lookup
		BoneNameToIndexMap.Add(Bone.BoneName, RigBone.Index);
	}
}
	...
	const TArrayView<FPowerIKBoneLimit>& JointLimits)
{
	...
	for (int i = 0; i < ExcludedBones.Num(); ++i)
	{
		...
		// FIX: Temp disable because logic error
...
void FPowerIKCore::SetSolverBoneTransformsFromControlRig(FRigBoneHierarchy* Hierarchy)
{
	// update bone data in solver
	for (int i = 0; i < Solver->GetNumBones(); ++i)
	{
...
void FPowerIKCore::CopySolverOutputToControlRig(FRigBoneHierarchy* Hierarchy)
{
	for (int i = 0; i < Solver->GetNumBones(); ++i)
	{
		Vec3 Position;
